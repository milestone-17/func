 ...
 *
 * 策略:
 * 1. 主数据: 构建时抓取并打包进站点的 public/qqq-data.json (同源, 永不跨域)
...
export async function loadBundledQuotes(): Promise<BundledQuote | null> {
  try {
    const res = await fetch(`${BASE}qqq-data.json`, { cache: 'no-cache' })
...
const YAHOO_CHART = 'https://query1.finance.yahoo.com/v8/finance/chart/QQQ?range=2y&interval=1d'
...
export async function fetchLiveLatestPrice(symbol: string): Promise<number | null> {
  const direct = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?range=5d&interval=1d`
