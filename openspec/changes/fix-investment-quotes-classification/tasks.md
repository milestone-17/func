...
- [x] 6.4 提交推送上线，用 `func-backup-20260813-0907.json` 数据线上验证：现价批量拉到真实净值（非「—」）、存量自动分类生效、永久组合显示正确比例与偏差 — commit 49858da 已部署 GitHub Pages；线上 8 项检查全过（16 只基金真实净值回填、5 纳指/1 标普/3 债券/4 红利/3 其他分类、永久总市值 ¥4,589.82 非零、债券基金归债券）；测试数据在一次性浏览器中随关闭销毁，未写入线上
